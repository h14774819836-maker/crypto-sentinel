...
async def run_worker() -> None:
    ...
    if market_config.enabled and market_config.api_key:
    ...
    youtube_config = settings.resolve_llm_config("youtube") if settings.youtube_enabled else None
    ...
    runtime = WorkerRuntime(
        ...
        sem_youtube=asyncio.Semaphore(max(1, int(getattr(youtube_config, "max_concurrency", 2) or 2))) if youtube_config else asyncio.Semaphore(1),
    ...
    if settings.telegram_enabled and settings.telegram_bot_token and inbound_mode == "polling":
