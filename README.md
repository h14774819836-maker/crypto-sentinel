...
Docker Compose 会优先使用 `.env` 里的 `DATABASE_URL`。保持默认的 `sqlite:///./data/crypto_sentinel.db` 时，容器会直接复用现有 `data/crypto_sentinel.db` 历史数据；只有在你显式把 `DATABASE_URL` 改成 PostgreSQL 时，才会切到 `db` 服务。
